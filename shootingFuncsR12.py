...
def f1(y, lmd, F):
    ...
    return -1 / 4 * (2 * lmd * y - 5 * 1j * y ** 4) / Q(y, lmd, F) + mpmath.sqrt(Q(y, lmd, F))
...
LEst=4
def calcBoundaryValue(lmd,F):
    theta = (- 1 / 4 * (1 / 2) - 1 / 14 * (1 / 2)) * mp.pi
    y0 = LEst * mpmath.exp(1j * theta)
    ...
    return mpmath.re(vAll[-1] / zAll[-1])
...
def computeOneSolution(inData):
   ...
   try:
       FVal=mpmath.findroot(lambda F:calcBoundaryValue(lmd,F),FEst,solver="muller",maxsteps=100)
       return [lmd,FVal]
   except ValueError:
       return []
